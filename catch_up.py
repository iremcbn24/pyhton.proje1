...
while run:
   ...
   window.blit(sprite1, (x1, y1)) #ekrana karakter1 i yerleştiriyoruz
   window.blit(sprite2, (x2, y2)) #ekrana karakter ikiyi yerleştiriyoruz

   for e in event.get(): #burada event.get listesi içerisindeki tüm öğeler tek tek kontrol edilir
       if e.type == QUIT: #eğer bu listedeki değerlerden bir tanesi QUIT'e eşitse yani x e basılmışsa 
           run = False #oyunu sonlandırdık

   keys_pressed = key.get_pressed() #key_pressed değişkenine tuşlara basılma olaylarını atadık
